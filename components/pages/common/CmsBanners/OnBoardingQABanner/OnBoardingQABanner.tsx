...
export const OnBoardingQABanner: React.FC<OnBoardingQABannerProps> = ({ data }) => {
  const [itemsState, setItemsState] = useState(() =>
    data.items.map((item, index) => ({ key: index, isActive: false, ...item }))
  )
  const { title } = data
            ...
            return (
              <div key={index} className="flex flex-col items-center justify-center">
                <button
                  ...
                  className="border border-[#e5e7eb] mb-[0.6rem] w-full min-h-[3.75rem] shadow-[0_0_20px_rgba(0,0,0,.05)] flex text-2xl text-text-primary justify-between items-center cursor-pointer"
                >
