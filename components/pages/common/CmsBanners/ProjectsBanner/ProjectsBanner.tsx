    ...
    <Container>
      <div className="flex flex-col items-center relative">
          ...
          {projects.map(({ description, image, title }, index) => {
            return (
              <div className={s.blogItem} key={index}>
                <div className="w-[220px] h-[80px] flex justify-center mt-8 "><Image style={{ "objectFit": 'contain' }} src={image?.url || ''} alt={image?.alternativeText || ''} width={image?.width} height={image?.height} /></div>
                <span className="text-xl text-text-primary font-bold my-4 pb-4 text-center">{title}</span>
                <span className="uppercase text-heading pb-4">{description}</span>
              </div>)
          })}
        </div>
        <div className={s.blog}>
          ...
          <div className="w-[50%] flex flex-col pl-36">
            <span className="font-secondary text-5xl font-bold text-[#fff] mb-2">{title}</span>
