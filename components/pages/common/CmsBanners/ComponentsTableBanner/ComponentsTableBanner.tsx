  ...
  const mobileTabData = useCallback((name: string) => {
    ...
    return (
      <div className="grid grid-cols-[1fr] px-4 py-2 border border-gray-6 w-full">
        {tabData.map((text) => (
          <div key={text} className="min-h-[3rem] flex justify-start pb-4 pt-3 border-b-[1px] border-gray-6">
            {text}
          </div>
        ))}
      </div>
        ...
        <div className="hidden md:grid grid-cols-[repeat(4,1fr)] text-text-primary w-full ">
          {headers.map((text, index) => {
            return (
              <div
                key={index}
                className="border border-gray-6 px-1 min-h-[3rem] font-semibold fron-bold flex items-center justify-center"
              >
                {text}
              </div>
            )
