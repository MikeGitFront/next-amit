...
export const ServicesBanner: React.FC<ServicesBannerProps> = ({ data }) => {
  ...
  const content = useCallback(
    (tabId: string) => {
      ...
      const tabsContent = tabs.reduce(
        (acc, tab) => ({ ...acc, [tab.title]: tab.content }),
        {} as Record<string, ServicesBannerData['tabs'][0]['content']>
      )
        ...
        {headings.map(({ title, image }, index) => {
          return (
            <>
              ...
              {!md && title === selectedTab && content(title)}
            </>
          )
        })}
      </div>
      {md && <div>{content(selectedTab)}</div>}
    </Container>
  )
}
